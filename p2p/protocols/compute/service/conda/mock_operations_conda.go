...
// This version is available when building with -tags conda for comparison tests
...
func (m *MockCondaOperations) RunPython(ctx context.Context, env, code string, stdin io.Reader) (*CommandExecution, error) {
	...
	} else if strings.Contains(code, "print('Hello, World!')") {
		// Match real output format
		output = "Hello, World!\n"
		exitCode = 0
	} else if strings.Contains(code, "sys.stdin.read()") {
		// Handle stdin reading
		if stdin != nil {
			buf := make([]byte, 1024)
			n, _ := stdin.Read(buf)
			input := strings.TrimSpace(string(buf[:n]))
			output = fmt.Sprintf("stdin: %s\n", input)
		} else {
			output = "stdin: \n"
		}
		exitCode = 0
...
func (m *MockCondaOperations) RunScript(ctx context.Context, env, scriptPath string, args []string, stdin io.Reader) (*CommandExecution, error) {
	...
	if env != "base" {
		if _, ok := m.Environments[env]; !ok {
			// Environment not found - return error
			return nil, fmt.Errorf("environment not found: %s", env)
		}
	}
