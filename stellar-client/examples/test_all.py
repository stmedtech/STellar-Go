#!/usr/bin/env python3
"""
Real Stellar Client Testing Guide

This script provides comprehensive examples of how to test all protocols
with a real Stellar node running from stellar-go.
"""
...
from stellar_client.models.responses import ScriptConfig
...
def setup_instructions():
    """Print setup instructions for running with real Stellar node."""
    print("""
🌟 Stellar Real Client Testing Setup
================================

STEP 1: Start the Stellar Node
---------------------------
In the stellar-go directory, run:

    go run cmd/stellar/main.go node

This will start the Stellar node and create a Unix socket at:
...
def test_connection():
    """Test basic connection to Stellar node."""
    ...
    try:
        ...
        print("\nTroubleshooting:")
        print("1. Make sure the Stellar node is running: go run cmd/stellar/main.go node")
...
def test_echo_protocol_real():
    ...
    try:
        with StellarClient() as client:
            ...
            devices = client.list_devices()
            if not devices:
                print("⚠ No devices found. Start more Stellar nodes to test inter-device communication.")
                return
...
def test_file_protocol_real():
    """Test file protocol with real Stellar node."""
        ...
        with StellarClient() as client:
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    test_file = Path(tmpdirname) / "stellar_test.txt"
                    
                    with open(test_file, "w") as f:
                        f.write("Hello from Stellar client!")
                    f.close()
                    
                    success = file_protocol.upload_file(
                        target_device, 
                        str(test_file), 
                        "stellar_uploaded.txt"
                    )
                    ...
                    success = file_protocol.download_file(
                        target_device,
                        "/tmp/stellar_uploaded.txt",
                        "/tmp/stellar_downloaded.txt"
                    )
                    print(f"✓ Download test file: {'Success' if success else 'Failed'}")
                
                Path("/tmp/stellar_downloaded.txt").unlink(missing_ok=True)
...
def main():
    ...
    print("🚀 Stellar Real Client Testing Suite")
    print("=" * 50)
