...
This directory contains a comprehensive test suite for the DCFL Client, organized by protocol usage order and designed to prevent test duplication while ensuring complete coverage.

## Test Structure

The tests are organized following the **protocol usage order** as defined in the DCFL client:
...
### Unit Tests
- Test individual protocol functionality without requiring a running DCFL node
- Focus on initialization, validation, and error handling
- Run quickly and don't require external dependencies

### Integration Tests
- Test complete workflows with a running DCFL node
- Marked with `@pytest.mark.integration`
- Require proper DCFL node setup and network connectivity
...
- `@pytest.mark.integration`: Integration tests requiring running DCFL node
...
- `mock_dcfl_client`: Mock client (removed)
...
### 1. No Mock Data
- All tests use real DCFL client instances
- Integration tests require actual DCFL nodes
...
- **Integration Tests**: May be skipped if no DCFL nodes are available
- **Total Coverage**: All DCFL client functionality should be covered

### Troubleshooting

1. **Integration tests skipped**: Ensure DCFL nodes are running
