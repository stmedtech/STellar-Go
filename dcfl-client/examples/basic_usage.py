#!/usr/bin/env python3
"""
Basic DCFL Client Usage Example

This example demonstrates the fundamental operations with DCFL client:
- Connecting to a DCFL node
...
from dcfl_client.exceptions import DCFLException

def get_first_device(client: DCFLClient):
    return next(iter(client.list_devices()), None)

def main():
    print("DCFL Client - Basic Usage Example")
    print("=" * 40)
    
    try:
        # Initialize client and connect to DCFL node
        with DCFLClient() as client:
            ...
            if not devices:
                print("No devices found. Start additional DCFL nodes to see them here.")
                return
        ...
        print("✓ Disconnected from DCFL node")
        
    except DCFLException as e:
        print(f"DCFL Error: {e}")
        return 1
    except Exception as e:
